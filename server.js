...
const db = new sqlite3.Database('./artwalk.db', (err) => {
  if (err) {
...
function seedDatabase() {
  ...
  db.all("SELECT COUNT(*) as count FROM artworks", (err, row) => {
    if (err || row.count === 0) {
...
app.get('/api/artworks/:id', (req, res) => {
  ...
  db.get("SELECT * FROM artworks WHERE id = ?", [id], (err, row) => {
    ...
    db.all("SELECT f.*, u.name as user_name FROM feelings f LEFT JOIN users u ON f.user_id = u.id WHERE f.artwork_id = ?", [id], (err, feelings) => {
      const artwork = {
        ...row,
        desc: row.desc,
        tags: row.tags ? row.tags.split(',') : [],
